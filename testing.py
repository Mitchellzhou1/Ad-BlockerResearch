...
class Driver:
    ...
    def wait_until_loaded(self, timeout=45, period=0.25, min_time=0):
    ...
    def scroll(self, speed=0.1):
        ...
        while True:
            ...
            scroll_step = 80  # Adjust this value to control the scroll speed
    ...
    def test_button(self, tries):
        ...
        if not os.path.exists(
                f"/home/mitch/work/pes/measurements/break/html_elements/test_ss/{self.url_key}-{self.image_index}.png"):
        ...
        thirteen_initial = self.get_local_DOM(element, 13)

        self.click_button(element)

        thirteen_after = self.get_local_DOM(element, 13)
        ...
        if check == "True - Redirect":
            ...
            self.excel_list.append([check, self.outer_HTML_changed, self.DOM_changed, self.initial_outer_html, '',
                                    self.initial_local_DOM[:300], self.after_local_DOM[:300], '', '', tries])
            ...
            self.excel_list.append([check, "False", "False", self.initial_outer_html, '',
                                    thirteen_initial, thirteen_after, '', '', tries])
            self.result = ''
    ...
    def get_elements(self):
        ...
        if self.html_obj == "login":
            ...
            for elem in ret:
                ...
                if outerHTML not in unique:
                    if self.html_obj != "input":
                        if not self.filter(elem):
                            continue
                    unique.append(outerHTML)
                    total_valid_elems += 1
