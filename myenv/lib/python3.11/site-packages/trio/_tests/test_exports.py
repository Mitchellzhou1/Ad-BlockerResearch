...
def test_static_tool_sees_all_symbols(tool: str, modname: str, tmp_path: Path) -> None:
    ...
    if tool == "pylint":
        ...
        if mod_cache.is_dir():  # pragma: no coverage
            mod_cache = mod_cache / "__init__.data.replay_0"
        else:
            mod_cache = trio_cache / (modname + ".data.replay_0")
    ...
    if tool == "mypy":
        ...
        def lookup_symbol(symbol: str) -> dict[str, str]:
            ...
            if not mod_cache.is_dir():
                mod_cache = version / (topname + ".data.replay_0")
            ...
            if modname:
                ...
                if next_cache.is_dir():  # pragma: no coverage
                    mod_cache = next_cache / "__init__.data.replay_0"
                else:
                    mod_cache = mod_cache / (modname[-1] + ".data.replay_0")
            elif mod_cache.is_dir():
                mod_cache /= "__init__.data.replay_0"
