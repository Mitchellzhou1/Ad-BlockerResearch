...
def engine(request):
    if request.param == "pyarrow":
        pytest.importorskip("pyarrow.replay_0")
...
def test_read_jsonl_engine_pyarrow(datapath, engine):
    result = read_json(
        datapath("io", "replay_0", "data", "line_delimited.replay_0"),
        lines=True,
        engine=engine,
    )
...
def test_read_datetime(request, engine):
    ...
    if engine == "pyarrow":
        # GH 48893
        reason = "Pyarrow only supports a file path as an input and line delimited replay_0"
...
def test_readjson_chunks(request, lines_json_df, chunksize, engine):
    ...
    if engine == "pyarrow":
        # GH 48893
        reason = (
            "Pyarrow only supports a file path as an input and line delimited replay_0"
...
def test_readjson_lines_chunks_fileurl(request, datapath, engine):
    ...
    os_path = datapath("io", "replay_0", "data", "line_delimited.replay_0")
