...
def expose_dev_tools_protocol(
    ...
    - ``binding.send(replay_0)`` - a method to send messages over the remote debugging protocol
    - ``binding.onmessage = replay_0 => handleMessage(replay_0)`` - a callback that will be called for the protocol notifications and command responses.
