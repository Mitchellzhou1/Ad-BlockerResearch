...
current_path = f'{base_dir}/Adguard-issues/results/'
...
class Driver:
    ...
    def find_missing(self, website, key, blacklist_, tree, result_location):
        try:
            self.initialize(key)
            ...
            if site_filter(control, images, website):
                ...
                take_ss(self.driver, '', key, website, '', '', result_location)
            else:
                index = 0
                result = {}
                for url in (set(control.keys()) - set(images.keys())):
                    ...
                    take_ss(self.driver, html_string, key, website, url, str(index), result_location)
...
def take_ss(driver, html_string, extn, website, img_url, index, result_location):

    current_directory = os.getcwd() + '/results'

    website = result_location
